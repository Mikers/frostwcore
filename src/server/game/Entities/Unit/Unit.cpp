...
bool Unit::HandleAuraProc(Unit * pVictim, uint32 damage, Aura * triggeredByAura, SpellEntry const * procSpell, uint32 /*procFlag*/, uint32 procEx, uint32 /*cooldown*/, bool * handled)
{
    ...
    switch(dummySpell->SpellFamilyName)
    {
	        ...
	        {
	            // Nether Shock
	            if (dummySpell->SpellFamilyFlags[1] & 10000000)
	            {
	                if (this->ToCreature()->isPet())
	                {
	                    *handled = true;
	                    this->CastSpell(pVictim, 62347, true);
	                    return true;
	                }
	            }
	            break;
...
uint32 Unit::SpellDamageBonus(Unit *pVictim, SpellEntry const *spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
	...
	/*	}
		if(isPet())
		{
			PetSpellBonusEntry const *pbonus = spellmgr.GetPetSpellBonusData(spellProto->Id);
			DoneTotal+=(damagetype == DOT?pbonus->pet_dot_damage:pbonus->pet_direct_damage)*this->GetTotalAttackPowerValue(BASE_ATTACK)+
				(damagetype == DOT?pbonus->master_dot_damage:pbonus->master_direct_damage)*((Pet*)this)->GetOwner()->GetTotalAttackPowerValue(BASE_ATTACK);
    */  }
            ...
            if (pVictim)
            {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    ...
                    case SPELLFAMILY_SHAMAN:
                        ...
                        {
                            if (!pVictim->HasAura(53480)) // Can't crit on Roar of Sacrifice
