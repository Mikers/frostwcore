...
void BattleGround::Update(uint32 diff)
{
    ...
    if (GetStatus() == STATUS_WAIT_JOIN && GetPlayersSize())
    {
        ...
        else if (GetStartDelayTime() <= 0 && !(m_Events & BG_STARTING_EVENT_4))
        {
            ...
            if (isArena())
            {
                ...
                m_ArenaTimeLimit = sWorld.getConfig(CONFIG_ARENA_TIME_LIMIT) * IN_MILLISECONDS; // this is a time limit for arena matches
    ...
    if (isArena() && GetStatus() == STATUS_IN_PROGRESS && m_ArenaTimeLimit)
    {
        ...
        if (m_ArenaTimeLimit <= 0)
        {
            ...
            for (itr = m_Players.begin(); itr != m_Players.end(); itr = next)
            {
                next = itr;
                ++next;
                RemovePlayerAtLeave(itr->first, true, true); // remove player from arena
            }
        }
    }
...
void BattleGround::EndBattleGround(uint32 winner)
{
    ...
    if (isArena() && isRated())
    {
        ...
        if (winner_arena_team && loser_arena_team && winner_arena_team != loser_arena_team)
        {
			...
			if(sWorld.getConfig(CONFIG_FAKEMMR) ==1)
				loser_change/=2;
            ...
            //uint32[m_ArenaType*2] DamageDone;
			sLog.outArena("Arena match Type: %u. Team1Id - %u, Team2Id - %u ended. WinnerTeamId - %u. RatingChange: %i.", m_ArenaType, m_ArenaTeamIds[BG_TEAM_ALLIANCE], m_ArenaTeamIds[BG_TEAM_HORDE], winner_arena_team->GetId(), winner_change);
			char DamageDone[255];
			for (BattleGroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
			{
				...
				if(winner_arena_team->GetType()>1 && winner_arena_team->HaveMember(itr->first) && winner_arena_team->GetStats().games_week > 100 &&
					winner_arena_team->GetStats().wins_week*100/winner_arena_team->GetStats().games_week>50 && 
					sWorld.getConfig(CONFIG_ARENA_MARK_OF_WIN_ENABLE) ==1)
				{
					ItemPosCountVec dest;
					if (plr->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, 43589, 1, false) == EQUIP_ERR_OK)
						plr->StoreNewItem(dest, 43589, 1, true);
				}
				int32 j  = snprintf( DamageDone, 255,     "; %u with ip %s", plr->GetGUIDLow(), plr->GetSession()->GetRemoteAddress().c_str() );
				if( winner_arena_team->HaveMember(itr->first))
					j += snprintf( DamageDone + j, 255 - j, "of team %u ", winner_arena_team->GetId() );
				else
					j += snprintf( DamageDone + j, 255 - j, "of team %u ", loser_arena_team->GetId() );
				j += snprintf( DamageDone + j, 255 - j, "dealed %u damage", _itr->second->DamageDone );
				sLog.outArena("%s", DamageDone);
			}
        }
        else
        {
    ...
    if (isArena() && isRated() && winner_arena_team && loser_arena_team && winner_arena_team != loser_arena_team)
    {
        ...
        if (sWorld.getConfig(CONFIG_ARENA_HISTORY) == 1)
        {
            CharacterDatabase.BeginTransaction();
            CharacterDatabase.PExecute("INSERT INTO arena_team_history (winner_team, winner_rating, loser_team, loser_rating, type, map) VALUES ('%u', '%u', '%u', '%u', '%u', '%u')",
            winner_arena_team->GetId(), winner_arena_team->GetRating(), loser_arena_team->GetId(), loser_arena_team->GetRating(), winner_arena_team->GetType(), GetMapId());
