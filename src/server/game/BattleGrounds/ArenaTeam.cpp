...
{
    if (int32(personal_rating) + (int32)mod < 0)
        personal_rating = 0;
    else
        personal_rating += (int32)mod;
    if (plr)
...
int32 ArenaTeam::LostAgainst(uint32 againstRating)
{
	...
	mod/=CustomMMR(againstRating);
    // modify the team stats accordingly
    FinishGame((int32)mod);
...
void ArenaTeam::MemberLost(Player * plr, uint32 againstRating)
{
    ...
    for (MemberList::iterator itr = m_members.begin(); itr !=  m_members.end(); ++itr)
    {
        if (itr->guid == plr->GetGUID())
        {
            ...
            int32 mod = (int32)ceil(K * (0.0f - chance));
			mod/=CustomMMR(againstRating);
            itr->ModifyPersonalRating(plr, int32(mod), GetSlot());
...
void ArenaTeam::OfflineMemberLost(uint64 guid, uint32 againstRating)
{
    ...
    for (MemberList::iterator itr = m_members.begin(); itr !=  m_members.end(); ++itr)
    {
        if (itr->guid == guid)
        {
            ...
            int32 mod = (int32)ceil(K * (0.0f - chance));
			mod/=CustomMMR(againstRating);
            if (int32(itr->personal_rating) + int32(mod) < 0)
                itr->personal_rating = 0;
            else
                itr->personal_rating += int32(mod);
            // update personal played stats
            itr->games_week +=1;
            itr->games_season +=1;
            return;
