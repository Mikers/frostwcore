...
/* void SpellMgr::LoadPetSpellBonusess()
{
	...
	uint32 count = 0;
	//                                                          0                   1             2                 3              4
	QueryResult_AutoPtr result = WorldDatabase.Query("SELECT entry, master_direct_damage, master_dot_damage, pet_direct_damage, pet_dot_damage FROM pet_spell_bonus_data");
	if( !result )
	{
		barGoLink bar( 1 );
		bar.step();
