        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
            {
                switch (GetId())
                {
					...
					case 66870: case 67621: case 67622: case 67623: //Burning Bite
						{
							target->RemoveAurasDueToSpell(66823); //Paralytic Toxin
							target->RemoveAurasDueToSpell(67618);
							target->RemoveAurasDueToSpell(67619);
							target->RemoveAurasDueToSpell(67620);
							break;
						}
                }
            }
        ...
        case SPELL_AURA_PERIODIC_TRIGGER_SPELL:
            switch (GetId())
            {
                ...
                case 70842: // Mana Barrier (hack)
                {
                    ...
                    if(uiHpDiff <= uiCurrentMana)
                    {
                        caster->ModifyHealth(uiHpDiff);
...
void AuraEffect::HandleAuraModResistanceExclusive(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    for (int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL; x++)
    {
        ...
        {
            // existing mods
			uint32 val = target->GetMaxPositiveAuraModifierByMiscMask(SPELL_AURA_MOD_RESISTANCE_EXCLUSIVE, 1<<x);
			uint32 val2= target->GetMaxPositiveAuraModifierByMiscMask(SPELL_AURA_MOD_RESISTANCE, 1<<x);
			target->SetModifierValue(UnitMods(UNIT_MOD_RESISTANCE_START + x), BASE_VALUE, val);
            if (target->GetTypeId() == TYPEID_PLAYER)
                target->SetResistanceBuffMods(SpellSchools(x),true,val+val2);
			target->UpdateResistances(x);
        }
    }
}
        ...
        if (apply)
        {
            ...
            switch(GetId())
            {
                ...
                case 67604: //Powering Up
                    if(GetBase()->GetStackAmount() >= 100)
                        GetBase()->Remove(AURA_REMOVE_BY_DEFAULT);
                    break;

					case 66870: //Burning Bite
						case 67621:
							case 67622:
								case 67623:
									target->RemoveAurasDueToSpell(66823); //Paralytic Toxin
            ...
            switch(m_spellProto->SpellFamilyName)
            {
		      ...
		      case 58730: // Restricted Flight Area
                   ...
                   if (sWorld.getConfig(CONFIG_OUTDOORPVP_WINTERGRASP_ENABLED)) 
                     {
                      if ((pvpWG != 0 && pvpWG->isWarTime()==false) || !target || ((target->GetAreaId() != 4581) && (target->GetAreaId() != 4539) && (target->GetAreaId() != 4197) && (target->GetAreaId() != 4585) && (target->GetAreaId() != 4612) && (target->GetAreaId() != 4582) && (target->GetAreaId() != 4583) && (target->GetAreaId() != 4589) && (target->GetAreaId() != 4575) && (target->GetAreaId() != 4538) && (target->GetAreaId() != 4577)))
                          break;
                      else
                      {
                      // Remove Flight Auras
