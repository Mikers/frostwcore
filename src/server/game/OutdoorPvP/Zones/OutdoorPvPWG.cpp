...
bool OutdoorPvPWG::SetupOutdoorPvP()
{
    ...
    //load worlstates
    QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT `entry`, `value` from `worldstates` where `entry` in ('31001','31002','31003') order by `entry`");
    
    m_WSSaveTimer = sWorld.getConfig(CONFIG_OUTDOORPVP_WINTERGRASP_SAVESTATE_PERIOD);
...
if (sWorld.getConfig(CONFIG_CONFIG_OUTDOORPVP_WINTERGRASP_ANTIFARM_ENABLE))
{
    if ((CountAtk<sWorld.getConfig(CONFIG_CONFIG_OUTDOORPVP_WINTERGRASP_ANTIFARM_ATK)) || (CountDef<sWorld.getConfig(CONFIG_CONFIG_OUTDOORPVP_WINTERGRASP_ANTIFARM_DEF)))
    {
