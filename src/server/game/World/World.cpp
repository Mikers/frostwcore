...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_configs[CONFIG_DURABILITY_LOSS_IN_PVP] = sConfig.GetBoolDefault("DurabilityLoss.InPvP", false);
		...
		m_configs[AnticheatHelpGM]              = sConfig.GetBoolDefault("Anticheat.HELP.GM",false);
	...
	m_configs[CONFIG_DUEL_RESET_COOLDOWN] = sConfig.GetIntDefault("DuelResetCooldown", 0);
	...
	m_configs[CONFIG_ANTIDODGE]                          = sConfig.GetBoolDefault("AntiDodge", false);
	m_configs[CONFIG_ANTIHONOR]                       	 = sConfig.GetBoolDefault("AntiHonor", false);
	...
	m_configs[CONFIG_ARENA_MARK_OF_WIN_ENABLE]					= sConfig.GetIntDefault ("Arena.MarkofWin.Enable", 0);
    m_configs[CONFIG_ARENA_SEASON_IN_PROGRESS]                  = sConfig.GetBoolDefault("Arena.ArenaSeason.InProgress", true);
    m_configs[CONFIG_ARENA_TIME_LIMIT]                          = sConfig.GetIntDefault ("Arena.ArenaTimeLimit", 0);
    m_configs[CONFIG_FAKEMMR]                                   = sConfig.GetBoolDefault("Arena.FAKEMMR", 0);
	m_configs[CONFIG_ARENA_HISTORY]                         	= sConfig.GetBoolDefault("Arena.History", 0);
    ...
    m_configs[CONFIG_OUTDOORPVP_WINTERGRASP_SAVESTATE_PERIOD] = sConfig.GetIntDefault("OutdoorPvP.Wintergrasp.SaveState.Period", 1000);
    ...
    m_configs[CONFIG_CONFIG_OUTDOORPVP_WINTERGRASP_ANTIFARM_ENABLE]  = sConfig.GetIntDefault("OutdoorPvP.Wintergrasp.Antifarm.Enable", 0);
    m_configs[CONFIG_CONFIG_OUTDOORPVP_WINTERGRASP_ANTIFARM_ATK]  = sConfig.GetIntDefault("OutdoorPvP.Wintergrasp.Antifarm.Atk", 5);
    m_configs[CONFIG_CONFIG_OUTDOORPVP_WINTERGRASP_ANTIFARM_DEF]  = sConfig.GetIntDefault("OutdoorPvP.Wintergrasp.Antifarm.Def", 5);

    m_configs[CONFIG_SHOW_INFO_MUTE_PLAYER] = sConfig.GetIntDefault("ShowInfoPlayer.mute", 0);
    m_configs[CONFIG_SHOW_INFO_KICK_PLAYER] = sConfig.GetIntDefault("ShowInfoPlayer.kick", 0);
    m_configs[CONFIG_SHOW_INFO_BAN_PLAYER] = sConfig.GetIntDefault("ShowInfoPlayer.ban", 0);

    m_configs[CONFIG_START_COMMAND_SEND_PLAYER_TO_GRAVEYAR] = sConfig.GetIntDefault("StartCommandSendToGraveyar", 0);
	...
	m_configs[CONFIG_CUSTOM_COMMADS] = sConfig.GetIntDefault("CustomCommandsTimer", 30000);
}
...
/*	sLog.outString("Loading Pet Spell Bonus Data...");
	...
	m_timers[WUPDATE_CUSTOM_COMMANDS].SetInterval(m_configs[CONFIG_CUSTOM_COMMADS]);
...
void World::Update(uint32 diff)
{
	...
	if (m_timers[WUPDATE_CUSTOM_COMMANDS].Passed())
	{
		if (m_timers[WUPDATE_CUSTOM_COMMANDS].GetInterval() && m_timers[WUPDATE_CUSTOM_COMMANDS].Passed())
		SendCustomCommands();
	}
...
void World::SendCustomCommands()
{
	QueryResult_AutoPtr result = CharacterDatabase.PQuery("Select * from custom_commands where isactive='1'");
	if(!result)
		return;
	if(Field *fields = result->Fetch()) do
	{
		if( time_t time1 = fields[2].GetUInt64())
			if(time1 >= time_t(time(NULL)))
				continue;
		const char * commands = fields[1].GetString();
        CliCommandHolder * cch = new CliCommandHolder(NULL,commands,&RASocket::zprint,NULL);
		QueueCliCommand(cch);
		zprint_=&RASocket::zprint;
		uint32 guid = fields[0].GetUInt32();
		CharacterDatabase.PExecute("Update `custom_commands` set isactive='0' where id='%u'", guid);
		commandq.push_back(guid);
	}
	while(result->NextRow());
}
