 	...
 	bool WorldSession::Anti__ReportCheat(const char* Reason,float Speed,const char* Op,float Val1,uint32 Val2,MovementInfo* MvInfo)
  	{
 	    ...
 	    const char* Player=GetPlayer()->GetName();
  	    ...
  	    if(!Player)
  	    {
  	        sLog.outError("Anti__ReportCheat: Player not found!");
  	        return false;
  	    }
	    ...
	    if(Res)
	    {
			...
			if (sWorld.getConfig(AnticheatHelpGM))
  	    ...
  	    else
  	    {
            ...
            << GetPlayer()->GetPositionZ();
  	        if(MvInfo)
  	        {
  	            Pos << "\nNew: " << MvInfo->t_pos.GetPositionX() << " " << MvInfo->t_pos.GetPositionY() << " " << MvInfo->t_pos.GetPositionZ() << "\n"
                 << "Flags: " << MvInfo->flags << "\n"
...
void WorldSession::HandleMoveWorldportAckOpcode()
{
    ...
    if (_player->InBattleGround())
    {
        ...
        else if (BattleGround *bg = _player->GetBattleGround())
        {
			...
			if( bg->isArena() && sWorld.getConfig(CONFIG_ANTIDODGE))
...
void WorldSession::HandleMovementOpcodes(WorldPacket & recv_data)
{
   ...
   /* if (recv_data.size() != recv_data.rpos())
    ...
    if (movementInfo.flags & MOVEMENTFLAG_ONTRANSPORT)
    {
        ...
        {
	            float trans_rad = movementInfo.t_pos.GetPositionX()*movementInfo.t_pos.GetPositionX() + movementInfo.t_pos.GetPositionY()*movementInfo.t_pos.GetPositionY() + movementInfo.t_pos.GetPositionZ()*movementInfo.t_pos.GetPositionX();
