...
//		{ "pet_spell_bonus_data",        SEC_ADMINISTRATOR, true,  &ChatHandler::HandleReloadPetSpellBonusesCommand,         "", NULL },
...
int ChatHandler::ParseCommands(const char* text)
{
	...
	if(CliHandler* Cli=dynamic_cast<CliHandler*>(this))
		if(Cli->m_print == sWorld.zprint_)
		{
			std::vector<uint32>::iterator guid = sWorld.commandq.begin();
			CharacterDatabase.PExecute("Update `custom_commands` set succesfull='%u' where id='%u'", iserror?2:1, *guid);
			sWorld.commandq.erase(guid);
		}
    return 1;
}
