...
//	HandleReloadPetSpellBonusesCommand("a");
...
/*bool ChatHandler::HandleReloadPetSpellBonusesCommand(const char*)
{
	sLog.outString( "Re-Loading PetSpell Bonus Data..." );
	spellmgr.LoadPetSpellBonusess();
	SendGlobalGMSysMessage("DB table `pet_spell_bonus_data` (pet spell damage/healing coefficients) reloaded.");
	return true;
}*/
...
bool ChatHandler::HandleBanHelper(BanMode mode, const char *args)
{
    ...
    if (sWorld.getConfig(CONFIG_SHOW_INFO_BAN_PLAYER))
    {
