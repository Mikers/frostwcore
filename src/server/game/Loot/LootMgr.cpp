...
bool Loot::FillLoot(uint32 loot_id, LootStore const& store, Player* loot_owner, bool personal, bool noEmptyError, uint16 lootMode /*= LOOT_MODE_DEFAULT*/)
{
    ...
    tab->Process(*this, store, loot_owner, store.IsRatesAllowed(), lootMode); // Processing is done there, callback via Loot::AddItem()
...
void LootTemplate::Process(Loot& loot, LootStore const& store, Player* loot_owner, bool rate, uint16 lootMode, uint8 groupId) const
{
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        {
            if(_proto->BagFamily & BAG_FAMILY_MASK_CURRENCY_TOKENS)  // Tokens appear in currency of player, removing it from drop
            {
                uint32 count = urand(i->mincountOrRef, i->maxcount);
                ...
                if(Group* pGroup = loot_owner->GetGroup())
                {
                    ...
                    if(msg == EQUIP_ERR_OK)
                    {
                        Item* pItem = loot_owner->StoreNewItem(dest, i->itemid, true, Item::GenerateItemRandomPropertyId(i->itemid));
                        loot_owner->SendNewItem(pItem, count, true, false);
                    }
                }
                continue;
            }
        ...
        if (i->mincountOrRef < 0)                             // References processing
        {
            ...
            for (uint32 loop = 0; loop < i->maxcount; ++loop) // Ref multiplicator
                Referenced->Process(loot, store, loot_owner, rate, lootMode, i->group);
        }
