...
struct boss_lady_deathwisperAI : public ScriptedAI
{
	...
	void RandomSpawn()
	{
		...
		{
		    case 0:
		        me->SummonCreature(CREATURE_FANATIC,-619.006,2158.104,50.848,0,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,10000);
		        me->SummonCreature(CREATURE_ADHERENT,-598.697,2157.767,50.848,0,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,10000);
		        me->SummonCreature(CREATURE_FANATIC,-577.992,2156.989,50.848,0,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,10000);
		        break;
		    case 1:
		        me->SummonCreature(CREATURE_ADHERENT,-618.748,2266.648,50.849,0,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,10000);
		        me->SummonCreature(CREATURE_FANATIC,-598.573,2266.870,50.849,0,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,10000);
		        me->SummonCreature(CREATURE_ADHERENT,-578.360,2267.210,50.849,0,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,10000);
                break;
        }
	}
	...
	void UpdateAI(const uint32 uiDiff)
	{
		...
		if (m_uiPhase == 1)
		{
			...
			if (m_uiSummonWaveTimer < uiDiff)
			{
				...
				if (getDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL || getDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC)
				{
					...
					me->SummonCreature(CREATURE_FANATIC,-598.573,2266.870,50.849,0,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,10000);
					me->SummonCreature(CREATURE_ADHERENT,-578.360,2267.210,50.849,0,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,10000);
					me->SummonCreature(RAND(CREATURE_FANATIC, CREATURE_ADHERENT), -555.643,2211.281,49.476,0,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,10000);
				}
				m_uiSummonWaveTimer = 60000;
		...
		if (m_uiPhase == 2)
		{
			...
			if (getDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL || getDifficulty() == RAID_DIFFICULTY_10MAN_HEROIC || getDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC)
            {
...
struct npc_shadeAI : public ScriptedAI //Dont work , need correct!
{
	...
	void Reset()
	{
		...
		if (pTarget && pTarget->IsInDist2d(me->GetPositionX(), me->GetPositionY(), 3.0f) && Despawn == false)
        {
