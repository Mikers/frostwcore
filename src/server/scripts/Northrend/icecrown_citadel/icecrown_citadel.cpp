...
enum Spells
{
	...
	SPELL_DEATHCHILLBLAST_10_NORMAL   = 70594,
	SPELL_DEATHCHILLBLAST_25_NORMAL   = 72005,
	SPELL_DEATHCHILLBLAST_10_HEROIC   = 72488,
	SPELL_DEATHCHILLBLAST_25_HEROIC   = 72489,
	SPELL_DEATHCHILLBLAST_EM_10_NORMAL = 70906,
	SPELL_DEATHCHILLBLAST_EM_25_NORMAL = 72485,
	SPELL_DEATHCHILLBLAST_EM_10_HEROIC = 72486,
	SPELL_DEATHCHILLBLAST_EM_25_HEROIC = 72487,
	SPELL_SHROUDOFTHEOCCULT           = 70768,
	SPELL_DARKMARTYRDOM_FANATIC       = 71236,
	SPELL_DARKMARTYRDOM_ADHERENT      = 71500,
	SPELL_DARKTRANSFORMATION          = 70900,
	SPELL_NECROTICSTRIKE_10_NORMAL    = 70659,
	SPELL_NECROTICSTRIKE_25_NORMAL    = 72490,
	SPELL_NECROTICSTRIKE_10_HEROIC    = 72490,
	SPELL_NECROTICSTRIKE_25_HEROIC    = 72492,
	SPELL_SHADOWCLEAVE_10_NORMAL      = 70670,
	SPELL_SHADOWCLEAVE_25_NORMAL      = 72006,
	SPELL_SHADOWCLEAVE_10_HEROIC      = 72493,
	SPELL_SHADOWCLEAVE_25_HEROIC      = 72494,
...
struct  npc_death_bound_wardAI: public ScriptedAI
{
	...
	void UpdateAI(const uint32 diff)
	{
		...
		{
			DoCast(me, SPELL_SABER_SLASH);
			m_uiSlashTimer = 6000;
		} else m_uiSlashTimer -= diff;
...
struct  npc_death_speaker_discipleAI: public ScriptedAI
{
	...
	void UpdateAI(const uint32 diff)
	{
		...
		if (m_uiMendTimer  <= diff)
		{
			...
			if(pTarget && pTarget->GetHealth()*100 / pTarget->GetMaxHealth() < 30 && pTarget->IsFriendlyTo(me))
			{
...
struct  npc_cult_adherentAI: public ScriptedAI
{
	...
	void UpdateAI(const uint32 diff)
	{
		...
		{
			if (Empowered == false && Resurrect == false)
			{
		...
		if (m_uiDarkTimer <= diff)
		{
		    if(Resurrect == false)
			{
			    DoCast(me, SPELL_DARKEMPOWERMENT);
			    m_uiDarkTimer = 20000;
			}
		} else m_uiDarkTimer -= diff;
		...
		if (m_uiDeathTimer <= diff)
		{
			...
			if (!Empowered)
			{
				DoCast(pTarget, RAID_MODE(SPELL_DEATHCHILLBLAST_10_NORMAL,SPELL_DEATHCHILLBLAST_25_NORMAL,SPELL_DEATHCHILLBLAST_10_HEROIC,SPELL_DEATHCHILLBLAST_25_HEROIC));
			}
			if (Empowered)
			{
				DoCast(pTarget, RAID_MODE(SPELL_DEATHCHILLBLAST_EM_10_NORMAL,SPELL_DEATHCHILLBLAST_EM_25_NORMAL,SPELL_DEATHCHILLBLAST_EM_10_HEROIC,SPELL_DEATHCHILLBLAST_EM_25_HEROIC));
			}
			m_uiDeathTimer = 7000;
...
struct  npc_cult_fanaticAI: public ScriptedAI
{
	...
	void Reset()
	{
		...
		m_uiVampirTimer = 25000;
		m_uiTransformationTimer = 30000;
	...
	void UpdateAI(const uint32 diff)
	{
		...
		if (m_uiTransformationTimer <= diff)
		{
			if(Transformation == false && Dark == false)
			{
				DoCast(me, SPELL_DARKTRANSFORMATION);
				Transformation = true;
			}
			m_uiTransformationTimer = 30000;
		} else m_uiTransformationTimer -= diff;

		if (m_uiStrikeTimer <= diff)
		{
			DoCast(me, RAID_MODE(SPELL_NECROTICSTRIKE_10_NORMAL,SPELL_NECROTICSTRIKE_25_NORMAL,SPELL_NECROTICSTRIKE_10_HEROIC,SPELL_NECROTICSTRIKE_25_HEROIC));
			m_uiStrikeTimer = 8000;
		} else m_uiStrikeTimer -= diff;

		if (m_uiCleaveTimer <= diff)
		{
			DoCast(me, RAID_MODE(SPELL_SHADOWCLEAVE_10_NORMAL,SPELL_SHADOWCLEAVE_25_NORMAL,SPELL_SHADOWCLEAVE_10_HEROIC,SPELL_SHADOWCLEAVE_25_HEROIC));
			m_uiCleaveTimer = 6000;
		} else m_uiCleaveTimer -= diff;
