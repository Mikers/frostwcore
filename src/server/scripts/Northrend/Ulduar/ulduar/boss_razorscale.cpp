...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_expedition_commander_ulduarAI(pCreature);
    }
struct npc_expedition_commander_ulduarAI : public ScriptedAI
{
    npc_expedition_commander_ulduarAI(Creature* pCreature) : ScriptedAI(pCreature), summons(me)
    {
        pInstance = pCreature->GetInstanceScript();
        greet = false;
    }
...
bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    switch(uiAction)
    {
        case GOSSIP_ACTION_INFO_DEF:
            ...
            CAST_AI(npc_expedition_commander_ulduarAI, (pCreature->AI()))->uiPhase = 1;
            break;
    }
    return true;
}
