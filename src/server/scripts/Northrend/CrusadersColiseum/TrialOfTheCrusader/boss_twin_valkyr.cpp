...
enum Says
{
    ...
    SAY_DVORTEX_L       = -1760046,
    SAY_DVORTEX_D       = -1760047,

    SAY_LVORTEX_L       = -1760048,
    SAY_LVORTEX_D       = -1760049,

    SAY_KILLING_L1      = -1760050,
    SAY_KILLING_D1      = -1760051,

    SAY_KILLING_L2      = -1760052,
    SAY_KILLING_D2      = -1760053,

    SAY_BERSERK_L       = -1760054,
    SAY_BERSERK_D       = -1760055,
...
{
    { 548.262085, 155.034042, 394.441650, 1.630077 },
    { 579.656372, 123.650337, 394.441559, 2.348724 }
};

Position portalSpawnsLight[2]      =
{
    { 580.662415, 156.519287, 394.442169, 3.928951 },
    { 546.694153, 122.910606, 394.441620, 0.775576 } 
};
...
struct boss_valkyr_twinAI : public ScriptedAI
{
    boss_valkyr_twinAI(bool light, Creature* creature) : ScriptedAI(creature), summons(me), isLight(light)
    {
        ...
        if (creature->GetMapId() == 649) // Trial of the Crusader
        {
            if (!isLight)
            {
                summons.Summon(DoSummon(34567, portalSpawnsDark[0]));
                summons.Summon(DoSummon(34567, portalSpawnsDark[1]));
            }
            else
            {
                summons.Summon(DoSummon(34568, portalSpawnsLight[0]));
    ...
    void DamageTaken(Unit* who, uint32& damage)
    {
        if (who->GetTypeId() == TYPEID_PLAYER)
        {
            if (who->HasAura(DARK_ESSENCE))
                isLight ? damage*=1.5f : damage/=2;
            if (who->HasAura(LIGHT_ESSENCE))
                isLight ? damage*=1.5f : damage+=damage/2;
        }
            ...
            : RAID_MODE(DARK_SHIELD_10N, DARK_SHIELD_25N, DARK_SHIELD_10H, DARK_SHIELD_25H)))
        {
            ...
            if (shield_amount >= RAID_MODE(175000, 700000, 300000, 1200000))
            {
    ...
    void DoTouch()
    {
        ...
        {
            if (isLight)
                DoCast(target, RAID_MODE(TOUCH_OF_LIGHT_10, TOUCH_OF_LIGHT_25, TOUCH_OF_LIGHT_10, TOUCH_OF_LIGHT_25));
            else
                DoCast(target, RAID_MODE(TOUCH_OF_DARKNESS_10, TOUCH_OF_DARKNESS_25, TOUCH_OF_DARKNESS_10, TOUCH_OF_DARKNESS_25));
        }
        if (GetTwin() && GetTwin()->isAlive())
        {
...
CreatureAI* GetAI_boss_light_twin(Creature* creature)
{
    return new boss_valkyr_twinAI(true, creature);
}

CreatureAI* GetAI_boss_dark_twin(Creature* creature)
{
...
CreatureAI* GetAI_mob_concentrated_light(Creature* creature)
{
    return new mob_concentrated_energieAI(true, creature);
}


CreatureAI* GetAI_mob_concentrated_dark(Creature* creature)
{
...
bool GossipHello_dark_essence(Player* player, Creature* /*creature*/)
{
...
bool GossipHello_light_essence(Player* player, Creature* /*creature*/)
{
...
void AddSC_boss_twins_valkyr()
{
    ...
    newscript->Name = "boss_light_twin";
    newscript->GetAI = &GetAI_boss_light_twin;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "boss_dark_twin";
    newscript->GetAI = &GetAI_boss_dark_twin;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "mob_concetrated_light";
    newscript->GetAI = &GetAI_mob_concentrated_light;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "mob_concetrated_dark";
    newscript->GetAI = &GetAI_mob_concentrated_dark;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "dark_essence";
    newscript->pGossipHello = &GossipHello_dark_essence;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "light_essence";
    newscript->pGossipHello = &GossipHello_light_essence;
    newscript->RegisterSelf();
}
