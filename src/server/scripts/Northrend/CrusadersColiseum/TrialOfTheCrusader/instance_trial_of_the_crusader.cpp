...
Position positionChestSpawn         = { 686.117249, 149.060455, 142.147736, 6.164101 };
...
struct instance_trial_of_the_crusader : public InstanceData
{
    instance_trial_of_the_crusader(Map* map) : InstanceData(map)
    {
    ...
    void OnPlayerEnter(Player* player)
    {
        ...
        {
            if (player->GetGroup())
				m_team = objmgr.GetPlayerTeamByGUID(player->GetGroup()->GetLeaderGUID());
            else
    ...
    void SetData(uint32 idx, uint32 data)
    {
        if (idx == SCARAB_DEATH)
        {
            ...
            {
                for (int i=0; i<scarabsDeaths.size(); ++i)
                {
                    time_t scarab1 = scarabsDeaths[i];
                    uint32 count = 0;

                    for (int j=0; j<scarabsDeaths.size(); ++j)
                    {
                        if (i == j)
                            continue;
    ...
    {
        if (loading)
            return InstanceData::SetBossState(id, state);

        if (id == DATA_FACTION_CHAMPIONS && state == DONE)
            instance->GetCreature(uiTirionGuid)->SummonGameObject(GO_CACHE, positionCenter, 0, 0, 0, 0, 0);
        ...
        if (id == DATA_ANUBARAK && state == DONE)
        {
                    ...
                    if (!list.isEmpty())
                    {
                        for (Map::PlayerList::const_iterator itr = list.begin(); itr != list.end(); ++itr)
                        {
                            ...
                            for (uint8 i = EQUIPMENT_SLOT_START; i < INVENTORY_SLOT_ITEM_END; ++i)
                            {
                                ...
                                {
                                    if (ItemPrototype const* proto = objmgr.GetItemPrototype(pItem->GetEntry()))
                                    {
            ...
            if (instance->GetSpawnMode() == 2) //10man hero
            {
                if (attempts >= 25 && attempts < 45)
                    instance->GetCreature(uiTirionGuid)->SummonGameObject(195665, positionChestSpawn, 0, 0, 0, 0, 0);
                else if (attempts >= 45 && attempts < 50)
                    instance->GetCreature(uiTirionGuid)->SummonGameObject(195666, positionChestSpawn, 0, 0, 0, 0, 0);
                else if (attempts >= 50)
                    instance->GetCreature(uiTirionGuid)->SummonGameObject(195667, positionChestSpawn, 0, 0, 0, 0, 0);
                else
                    instance->GetCreature(uiTirionGuid)->SummonGameObject(195668, positionChestSpawn, 0, 0, 0, 0, 0);
            }

            if (instance->GetSpawnMode() == 3) //25man hero
            {
                if (attempts >= 25 && attempts < 45)
                    instance->GetCreature(uiTirionGuid)->SummonGameObject(195669, positionChestSpawn, 0, 0, 0, 0, 0);
                else if (attempts >= 45 && attempts < 50)
                    instance->GetCreature(uiTirionGuid)->SummonGameObject(195670, positionChestSpawn, 0, 0, 0, 0, 0);
                else if (attempts >= 50)
                    instance->GetCreature(uiTirionGuid)->SummonGameObject(195671, positionChestSpawn, 0, 0, 0, 0, 0);
                else
                    instance->GetCreature(uiTirionGuid)->SummonGameObject(195672, positionChestSpawn, 0, 0, 0, 0, 0);
            }
    ...
    std::string GetSaveData()
    {
        ...
        saveStream << GetBossSaveData() << " " << deathsCount << "" << attempts;
...
InstanceData* GetInstanceData_trial_of_the_crusader(Map* map)
{
...
void AddSC_instance_trial_of_the_crusader()
{
    ...
    newscript->GetInstanceData = &GetInstanceData_trial_of_the_crusader;
