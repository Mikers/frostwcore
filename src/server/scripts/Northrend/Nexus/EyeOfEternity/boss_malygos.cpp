...
public:
    ...
    {
        if(instance)
            instance->SetData(TYPE_MALYGOS, NOT_STARTED);
        else
            me->ForcedDespawn();
    ...
    void JustReachedHome()
    {
        ...
        {
            pGo->Respawn();
            instance->SetData(TYPE_DESTROY_PLATFORM, NOT_STARTED);
        }
        else
    ...
    void EnterCombat(Unit* pWho)
    {
        ...
        instance->SetData(TYPE_MALYGOS, IN_PROGRESS);
        DoScriptText(SAY_AGGRO1, me);
 
        if(instance->GetData(TYPE_OUTRO_CHECK) == 1) //Should be enought to trigger outro immediatly
        {
            me->SetFlying(true);
            me->AddUnitMovementFlag(MOVEMENTFLAG_SPLINE_ELEVATION);
            //Destroy Platform
            CastSpellToTrigger(SPELL_DESTROY_PLATFROM_BOOM, false);            
            instance->SetData(TYPE_DESTROY_PLATFORM, IN_PROGRESS);
    ...
    {
        instance->SetData(TYPE_MALYGOS, DONE);
        instance->SetData(TYPE_OUTRO_CHECK, 0);
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if(m_uiPhase == PHASE_FLOOR)
        {
            if(m_uiSubPhase == SUBPHASE_VORTEX)
            {
                if(m_uiTimer <= uiDiff)
                {
                    ...
                    if(m_uiVortexPhase >= MAX_VORTEX && m_uiVortexPhase < 29)
                    {
                        ...
                        //PowerSpark(2);
                        if(instance)
                            instance->SetData(TYPE_VORTEX, 0);
            ...
            if(m_uiVortexTimer <= uiDiff)
            {
                //PowerSpark(3);
                if(instance)
                    instance->SetData(TYPE_VORTEX, 1);
        ...
        }else if(m_uiPhase == PHASE_ADDS)
        {
            ...
            if(m_uiTimer <= uiDiff)
            {
                ...
                {
                    if(instance)
                        instance->SetData(TYPE_PLAYER_HOVER, DATA_DROP_PLAYERS);
        ...
        else if(m_uiPhase == PHASE_OUTRO)
        {
            ...
            {
                instance->SetData(TYPE_OUTRO_CHECK, 1);
...
public:
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if(m_uiCheckTimer <= uiDiff)
        {
            if(instance && instance->GetData(TYPE_VORTEX))
            {
                me->GetMotionMaster()->MovementExpired();
                me->GetMotionMaster()->MoveIdle();
                return;
...
public:
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            if(instance->GetData(TYPE_MALYGOS) == NOT_STARTED)
                me->ForcedDespawn();
