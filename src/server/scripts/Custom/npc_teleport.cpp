...
#include "ScriptPCH.h"
#include "../../../game/Customs/ScNpcTeleport.h"
#include <sstream>
...
#define NEXT_PAGE               "-> [След. страница]"
#define PREV_PAGE               "<- [Пред. страница]"
...
namespace
{
    ...
    void ActionTeleport(Player * const player, Creature * const creature, const uint32 &id)
    {
        ...
        {
            std::string msg ("Вы не достигли нужного уровня. Эта телепортация требует уровень " + ConvertStr(dest.m_level) + ".");
        ...
        {
            std::string msg ("У вас не хватает денег. Стоимость телепортации составляет " + ConvertMoney(dest.m_cost) + ".");
            creature->MonsterWhisper(msg.c_str(), player->GetGUID());
            return;
        }
...
bool OnGossipHello(Player *player, Creature *creature)
{
    ...
    if (player->isInCombat())
    {
        ...
        creature->MonsterWhisper("Вы находитесь в бою. Попробуйте позже", player->GetGUID());
        return true;
    }
